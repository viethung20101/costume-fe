...
namespace FrostweepGames.Plugins.GoogleCloud.SpeechRecognition.V1
{
	public class SpeechRecognitionManager : IService, IDisposable, ISpeechRecognitionManager
	{
		...
		private Dictionary<string, string> GetHeaders()
		{
			...
			if(CurrentConfig.enabledAPIKeyValidation)
			{
#if UNITY_ANDROID
				headers.Add("X-Android-Package", CurrentConfig.packageName);
				headers.Add("X-Android-Cert", CurrentConfig.keySignature);
#elif UNITY_IOS
				headers.Add("X-Ios-Bundle-Identifier", CurrentConfig.packageName);
#endif
			}
			return headers;
		}
